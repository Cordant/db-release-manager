...
export class DatabaseFileHelper {
      ...
      if (lastVersion.versionName !== 'current') {
        ...
        while (!ok) {
          const response = await uiUtils.question({
            origin: this._origin,
            text: `The last version is not current (last version = ${versionToChange}). If you wish to amend this version, please use "y". If you want to create a new version (current folder), please use "c".`,
          });
          if (response === 'c') {
  ...
  static async getObjectName(objectName: string | undefined, objectType: string, databaseObject: DatabaseObject, uiUtils: UiUtils): Promise<string> {
    ...
    while (!found) {
      ...
      if (validTableNames.length === 1) {
        ...
        objectName = await uiUtils.question({
          origin: this._origin,
          text: `No ${objectType} could be found with the provided name. Please type a valid name`,
        });
      } else {
    ...
    while (!found) {
      ...
      if (validTableNames.length === 1) {
        ...
        dbName = await uiUtils.question({
          origin: this._origin,
          text: `No database could be found with the provided name. Please type a valid name`,
        });
      } else {
        // more than one object => we show a list of choices
  ...
  static async createFunctions(params: {
    ...
    for (let t = 0; t < tables.length; t++) {
      ...
      if (!databaseObject.table[tableName].tags.ignore) {
        ...
        const tablesToRetrieveAlong: {
          fieldName: string,
          table: DatabaseTable
        }[] = Object.keys(databaseObject.table).map(t => {
  ...
  static async createTable(params: {
    ...
    if (!params.tableDetails || (params.tableDetails && !params.tableDetails.name)) {
      ...
      let tableName = '';
      while (!tableName) {
        let newTableName = await uiUtils.question({
          origin: DatabaseFileHelper._origin,
          text: 'Please provide a table name',
        });
        if (!DatabaseFileHelper._checkNewNameHasUnderscoresAndAlphanumerics(newTableName)) {
          uiUtils.warning({
            origin: DatabaseFileHelper._origin,
            message: 'Please use snake cased alphanumeric characters',
          });
          tableName = '';
        } else {
      ...
      // ask about fields
      let finished = false;
      let newFieldName = await uiUtils.question({
        origin: DatabaseFileHelper._origin,
      ...
      while (!finished) {
        ...
        while (!validName) {
          ...
          if (!validName) {
            newFieldName = await uiUtils.question({
              origin: DatabaseFileHelper._origin,
              text: 'Please provide a field name',
            });
          }
        ...
        while (!selectedType) {
          selectedType = await uiUtils.question({
            origin: DatabaseFileHelper._origin,
            text: `Please select a type, or type the desired one : \n${types.map((x, i) => {
              return `\t ${i + 1} - ${x}`;
            }).join('\n')}`,
          });
        }
        ...
        params.tableDetails.fields[paramIndex].notNull = (await uiUtils.question({
          origin: DatabaseFileHelper._origin,
          text: 'Should it be not null ? (y for yes)',
        })).toLowerCase() === 'y';
        if (params.tableDetails.fields[paramIndex].notNull) {
          params.tableDetails.fields[paramIndex].default = (await uiUtils.question({
            origin: DatabaseFileHelper._origin,
            text: 'Please provide a default value (press enter for none)',
          }));
          if (!params.tableDetails.fields[paramIndex].default) {
            params.tableDetails.fields[paramIndex].default = undefined;
          }
        }
        params.tableDetails.fields[paramIndex].unique = (await uiUtils.question({
          origin: DatabaseFileHelper._origin,
          text: 'Should it be unique ? (y for yes)',
        })).toLowerCase() === 'y';

        // next
        newFieldName = await uiUtils.question({
          origin: DatabaseFileHelper._origin,
          text: 'Please provide the next field name',
        });
  ...
  private static async _checkNewTableName(databaseObject: DatabaseObject, tableName: string, uiUtils: UiUtils): Promise<string> {
    ...
    if (probablyHasSuffix) {
      hasSuffix = (await uiUtils.question({
        origin: DatabaseFileHelper._origin,
        text: 'It seems that you provided a suffix for this table, pleas confirm (y/n)',
      })) === 'y';
    }
    if (hasSuffix) {
      return tableName;
  ...
  static async createVersion(params: {
    ...
    if (!version) {
      ...
      version = (chosenVersion.version.match(/^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+/i) as RegExpMatchArray)[0];
    }

    if (params.version) {
      if (databaseObject._versions.includes(version)) {
        throw 'The provided version already exists, please choose another one';
      }
    }
  ...
  static async editObject(params: {
    ...
    uiUtils.success({
      origin: this._origin,
      message: `New file version created for ${params.objectType} "${params.objectName}"`,
    });
  ...
  static async renameTableField(params: {
    ...
    const databaseSubObject: DatabaseSubObject =
      await DatabaseHelper.getDatabaseSubObject({
        ...params,
        objectType: 'table',
      }, databaseObject, DatabaseFileHelper._origin, uiUtils);
